...
<template>
  <header class="institution-header sticky top-0 z-20 border-b border-emerald-100 bg-white/95 backdrop-blur-xl">
    <!-- Utility bar -->
    ...
    <div>
      <div class="mx-auto flex max-w-7xl items-center gap-8 px-6 py-4">
        <NuxtLink to="/" class="mr-auto flex min-w-0 shrink items-center gap-5 sm:gap-6" aria-label="E-LIBRARY home">
          <span class="flex h-12 w-12 shrink-0 items-center justify-center">
            <img src="/images/digital-library-logo.png" alt="DigitalLibrary" class="h-full w-full rounded-full object-contain" />
          </span>
          <span class="institution-wordmark min-w-0 pl-1">
            <span class="block truncate font-display text-xl font-black leading-none tracking-[-0.06em] text-emerald-900 sm:text-2xl">{{ libraryData.settings.libraryName }}</span>
            <span class="mt-1 block font-mono text-[8px] uppercase tracking-[0.18em] text-emerald-700">Digital knowledge centre</span>
          </span>
        </NuxtLink>
        ...
        <div v-else class="hidden sm:block relative">
          ...
          <Transition name="dropdown">
            ...
            <NuxtLink to="/account" class="block px-3.5 py-2 text-sm text-emerald-900 hover:bg-emerald-50"
              >My account</NuxtLink
            >
            <NuxtLink
              v-if="isAdmin"
      ...
      <Transition name="mobile-menu">
        <div v-if="mobileOpen" class="mobile-menu flex flex-col gap-3 border-t border-emerald-100 px-6 py-4 md:hidden">
        <NuxtLink
          v-for="link in links"
          :key="link.to"
          :to="link.to"
