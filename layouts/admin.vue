  ...
  <div class="admin-shell flex h-screen flex-col overflow-hidden bg-slate-50 text-slate-800">
    ...
    <div class="flex min-h-0 flex-1">
      <aside class="hidden h-full w-56 shrink-0 overflow-y-auto border-r border-slate-200 bg-white px-3 py-5 text-slate-800 shadow-sm lg:flex lg:flex-col">
        <nav class="flex-1 space-y-1"><NuxtLink v-for="item in navItems" :key="item.label" :to="item.to" class="relative flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-700 transition-all duration-200" :class="route.path === item.to ? 'bg-blue-50 text-blue-700 shadow-sm' : 'hover:bg-slate-100 hover:text-slate-900'"><component :is="item.icon" class="h-4 w-4" :class="route.path === item.to ? 'text-blue-700' : 'text-slate-500'" />{{ item.label }}</NuxtLink></nav>
        <div class="mt-5 space-y-1 border-t border-slate-200 pt-4"><NuxtLink to="/" class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900"><ChevronLeft class="h-4 w-4" /> Back to library</NuxtLink><button type="button" class="flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-red-50 hover:text-red-600" @click="onLogout"><LogOut class="h-4 w-4" /> Sign out</button></div>
      </aside>
