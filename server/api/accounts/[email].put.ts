...
export default defineEventHandler(async (event) => {
  ...
  if (approverEmail === email) {
    ...
    if (password && password.length < 6) {
      throw createError({ statusCode: 400, statusMessage: 'Password must contain at least 6 characters' })
    }
  ...
  if (name) account.name = name
  if (body?.role === 'super-admin' || body?.role === 'admin' || body?.role === 'user') account.role = body.role
