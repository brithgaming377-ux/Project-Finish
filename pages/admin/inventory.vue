...
const people = [
  { name: 'Sokha Kim', role: 'Student', loans: 3, status: 'Active' },
  { name: 'Dara Phan', role: 'Student', loans: 2, status: 'Active' },
  { name: 'Sreyneang Lim', role: 'Teacher', loans: 1, status: 'Active' },
  { name: 'Vuthy Chan', role: 'Student', loans: 0, status: 'New' }
]
  ...
  <div class="page-shell">
    <header class="flex flex-wrap items-start justify-between gap-5">
      <div>
        ...
        <h1 class="page-title">Library operations</h1>
      ...
      <div class="flex flex-wrap gap-3">
        <input
          ...
          @change="onImport"
        /><button
          class="rounded-xl border border-ink px-4 py-2.5 text-sm font-semibold hover:bg-ink hover:text-white"
          @click="fileInput?.click()"
        >
          Import catalog</button
        ><button
          class="rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-ink-light"
          @click="exportCatalog"
        >
          Export catalog
        </button>
    ...
    <div class="mt-9 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      >
        ...
        <p class="mt-2 font-display text-4xl font-semibold">{{ item.value }}</p>
        <p class="mt-2 font-mono text-[10px] uppercase tracking-wide text-sage">{{ item.note }}</p>
      </div>
    </div>
    <div class="mt-8 grid gap-8 lg:grid-cols-[1.2fr_.8fr]">
      <section class="surface-card overflow-hidden">
        <div class="flex items-center justify-between border-b border-line p-5">
          <div>
            <h2 class="font-display text-xl font-semibold">Stock overview</h2>
            <p class="mt-1 text-sm text-ink-soft">Books that need attention.</p>
          </div>
          <NuxtLink to="/admin" class="text-sm font-semibold text-amber-deep hover:underline"
            >Manage books →</NuxtLink
          >
        </div>
        <div class="divide-y divide-line">
          <div v-for="book in books.slice(0, 6)" :key="book.id" class="flex items-center gap-3 p-4">
            <BookCoverImage
              :src="book.coverUrl"
            ...
            <span
              class="rounded-full px-2 py-1 font-mono text-[10px]"
              :class="
      ...
      <section class="surface-card overflow-hidden">
        <div class="border-b border-line p-5">
          <h2 class="font-display text-xl font-semibold">People & loans</h2>
          <p class="mt-1 text-sm text-ink-soft">Recent library activity.</p>
        </div>
        <div class="divide-y divide-line">
          <div v-for="person in people" :key="person.name" class="flex items-center gap-3 p-4">
            <span
              class="flex h-10 w-10 items-center justify-center rounded-full bg-ink font-display text-sm font-semibold text-white"
              >{{ person.name.charAt(0) }}</span
            >
            <div class="flex-1">
              <p class="text-sm font-semibold">{{ person.name }}</p>
              <p class="text-xs text-ink-soft">
                {{ person.role }} · {{ person.loans }} active loans
              </p>
            </div>
            <span class="font-mono text-[10px] text-sage">{{ person.status }}</span>
          </div>
        </div>
      </section>
    </div>
